...
def run_one(
    ...
    result = {
        ...
        "score_predicted_accept": score_accepts(final_score, threshold=FIXED_THRESHOLD),
        "band_predicted_accept": decision_band_accepts(final_band),
        "score_correct": score_accepts(final_score, threshold=FIXED_THRESHOLD) == (row["accepted"] == 1),
        "band_correct": decision_band_accepts(final_band) == (row["accepted"] == 1),
        "score_band_consistent": score_accepts(final_score, threshold=FIXED_THRESHOLD) == decision_band_accepts(final_band),
